...
class Nalla:
    ...
    def loginform(self):
        ...
        self.Frame_login = Frame(self.root, bg="orange")
        self.Frame_login.place(x=0, y=0, height=700, width=1366)

        self.frame_input = Frame(self.root, bg="white")
        self.frame_input.place(x=10, y=10, height=380, width=330)

        label2 = Label(self.frame_input, text="Username", font=('Goody old style', 20, 'bold'), fg="orange", bg="white")
        label2.place(x=30, y=45)
        self.username_txt = Entry(self.frame_input, font=("times new roman", 15, "bold"), bg='white', bd=2)
        self.username_txt.place(x=30, y=95, width=270, height=35)

        label3 = Label(self.frame_input, text="Password", font=('Goody old style', 20, 'bold'), fg="orange", bg="white")
        label3.place(x=30, y=145)
        self.password = Entry(self.frame_input, font=("Goody old style", 15, "bold"), bg='white', bd=2)
        self.password.place(x=30, y=195, width=270, height=35)

        btn1 = Button(self.frame_input, text="forgot password?", command=self.change, cursor='hand2', font=('calibre', 10),
                      bg='white',
                      fg='black', bd=0)
        btn1.place(x=125, y=255)

        btn2 = Button(self.frame_input, text="Login", command=self.login, cursor="hand2", font=('times new roman', 15),
                      bg='orange', fg='white', bd=0, width=15, height=1)
        btn2.place(x=90, y=290)

        btn3 = Button(self.frame_input, text="Not Registered? register", command=self.Register, cursor='hand2',
                      font=('calibre', 10), bg='white', fg='black', bd=0)
        btn3.place(x=110, y=340)

        self.btn4 = Button(self.root, text="Back", command=root.destroy, cursor='hand2', font=('calibre', 12),
                      bg='orange', fg='white', bd=0)
        self.btn4.place(x=300, y=0, height='30', width='50')
        ...
        if self.username_txt.get() == "" or self.password.get() == "":
            ...
            try:
                ...
                cur.execute('select * from register where username=%s and password=%s',
                            (self.username_txt.get(), self.password.get()))
                row = cur.fetchone()
                if row == None:
    ...
    def change(self):
        ...
        label2 = Label(frame_input, text="old password", font=('Goody old style', 20, 'bold'), fg="orange", bg="white")
        label2.place(x=30, y=45)
        self.o_p = Entry(frame_input, font=("times new roman", 15, "bold"), bg='white', bd=2)
        self.o_p.place(x=30, y=95, width=270, height=35)
        label3 = Label(frame_input, text="new password", font=('Goody old style', 20, 'bold'), fg="orange", bg="white")
        label3.place(x=30, y=145)
        self.c_p = Entry(frame_input, font=("Goody old style", 15, "bold"), bg='white', bd=2)
        self.c_p.place(x=30, y=195, width=270, height=35)
    ...
    def confirm(self):
        try:
            con = pymysql.connect(host="localhost", user="root", password="Root", database="login")
            cur = con.cursor()
            cur.execute("update register set password=%s,confirmpassword=%s where password=%s",
                        (self.c_p.get(), self.c_p.get(), self.o_p.get()))
    ...
    def verification(self):
        if (self.entry5.get() == "") and (len(self.entry5.get()) != 10):
            messagebox.showerror("Error", "Enter Valid Contact number", parent=self.root)
        elif (self.entry6.get() == ""):
            messagebox.showerror("Error", "Enter your Email-ID", parent=self.root)
        else:
            o = smtplib.SMTP("smtp.gmail.com", 587)
            o.starttls()
            o.login("babakijaiho866@gmail.com", "Pass@1234")
            self.OTPE = random.randint(1000, 9999)
            message = "Your otp for registration is " + str(self.OTPE)
            listOfAddress = [self.entry6.get()]
            o.sendmail("babakijaiho866@gmail.com", listOfAddress, message)
            o.quit()

            account_sid = 'AC9877e8284bb5cf7852cfe668d8be3dfc'
            auth_token = '3946483477e56485ec5a0cc3b81f4894'

            client = Client(account_sid, auth_token)
            self.OTPS = str(random.randint(1000, 9999))
            client.messages.create(body=self.OTPS, from_='+12568587255', to="+918850483673", )
    ...
    def register(self):
        if self.entry.get() == "" or self.entry2.get() == "" or self.entry3.get() == "" or self.entry4.get() == "" \
            ...
            try:
                con = pymysql.connect(host="localhost", user="root", password="Root", database="login")
                cur = con.cursor()
                cur.execute("select * from register where emailid=%s", self.entry6.get())
                row = cur.fetchone()
                if row != None:
                    messagebox.showerror("Error register already exist ,please try with another Email",
                                         parent=self.root)
                else:
                    cur.execute("INSERT INTO register VALUES (%s,%s,%s,%s,%s,%s)",
                                (self.entry.get(), self.entry6.get(), self.entry2.get(), self.entry4.get(),
                                 self.entry5.get(), self.entry3.get()))
                    con.commit()
                    con.close()
                    # messagebox.showinfo("success", "Register Successful for:/n Username: %s/n Password: %s"
                    # + self.entry.get() + self.entry2.get(), parent=self.root)
