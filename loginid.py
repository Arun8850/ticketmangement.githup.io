...
class Nalla:
    ...
    def loginform(self):
        ...
        label2 = Label(frame_input, text="Username", font=('Goody old style', 20, 'bold'), fg="orange", bg="white")
        label2.place(x=30, y=45)
        self.username_txt = Entry(frame_input, font=("times new roman", 15, "bold"), bg='white', bd=2)
        self.username_txt.place(x=30, y=95, width=270, height=35)

        label3 = Label(frame_input, text="Password", font=('Goody old style', 20, 'bold'), fg="orange", bg="white")
        label3.place(x=30, y=145)
        self.password = Entry(frame_input, font=("Goody old style", 15, "bold"), bg='white', bd=2)
        self.password.place(x=30, y=195, width=270, height=35)

        btn1 = Button(frame_input, text="forgot password?",command=self.change, cursor='hand2', font=('calibre', 10), bg='white',
                      fg='black', bd=0)
    ...
    def register(self):
        if self.entry.get() == "" or self.entry2.get() == "" or self.entry3.get() == "" or self.entry4.get() == "" \
            ...
            try:
                ...
                row = cur.fetchone()
                if row != None:
                    messagebox.showerror("Error register already exist ,please try with another Email", parent=self.root)
                else:
